...
function [min_dist_fuzz] = GAFO(XYS,svname)
...
% Initialization of target map and associated variables
TARGET_X = XYS(:,1);
TARGET_Y = XYS(:,2);
n = length(XYS');
pop_size = 60;
...
fig = figure('Position',[(scrsz(3)-FIGURE_WIDTH)/2 ...
            ...
            FIGURE_WIDTH, FIGURE_HEIGHT],'visible','off');
...
%runs the straight line optimization
[slo_rte, min_dist_slo, slo_thetas] = slo_fuzz(XYS,radius,fuzzy_rte,opt_rte);
close('all');
writeOnFuzzy(slo_rte,min_dist_slo,fuzz,XYS,1);
